...
def dashboard():
    ...
    stock_total = conn.execute("""
        SELECT COUNT(*) FROM stock WHERE cantidad > 0
    """).fetchone()[0]
    ...
    stock_bajo = conn.execute("""
        ...
        WHERE p.activo = 1 AND p.stock_minimo > 0 AND COALESCE(s.cantidad, 0) <= p.stock_minimo
        ORDER BY stock
    """).fetchall()
    ...
    por_vencer = conn.execute("""
        ...
        WHERE p.activo = 1 AND p.vencimiento IS NOT NULL AND p.vencimiento != ''
          AND date(p.vencimiento) <= date(?, '+14 days')
    ...
    por_cobrar = conn.execute("""
        SELECT COALESCE(SUM(c.saldo), 0) AS total FROM creditos c WHERE c.estado = 'pendiente'
    """).fetchone()["total"]
